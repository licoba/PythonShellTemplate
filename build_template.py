...
AUTHOR_NAME = 'liocba'  # 作者
AUTHOR_EMAIL = 'licoba@163.com'  # 作者邮箱
README_URL = 'https://github.com/licoba/PythonShellTemplate/wiki'  # 说明文档

# =========================================== GLOBAL CONST ===========================================
TIMESTAMP = int(time.time())  # 当前时间戳，eg. 1617351251
# 当前时间，eg. 2021-04-02 16:14:11
DATE = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(TIMESTAMP))
DATE_STAMP = time.strftime('%Y%m%d%H%M%S', time.localtime(
    TIMESTAMP))  # 当前时间戳，eg. 20210402161411
...
def help():
    ...
    print_info('# Usage')
    print_info(
        '\t$ python %s [-dvh] [command] [params...] [--Option [value] [-sub_option [value]]...]...' % SCRIPT_NAME)
    print_info()
...
def main():
    opts_tuple, args = getopt.getopt(
        sys.argv[1:], 'dvh', ['help', 'verbose', 'version'])
    for opt, arg in opts_tuple:
        opts[opt.lstrip('-')] = arg
    for key in opts:
        opts[key] = opts[key] if opts[key].isspace() else True
    ...
    print_debug('SCRIPT_BASENAME_WITHOUT_SUFFIX = %s' %
                SCRIPT_BASENAME_WITHOUT_SUFFIX)
    ...
    for i in range(len(args)):
        print_debug('args[%s] = %s' % (i, args[i]))
    print_debug()
    print_debug(
        '======================== OPTS (%s) ======================' % len(opts))
    for key in opts:
        print_debug('opts[%s] = %s' % (key, opts[key]))
    print_debug()
    print_debug('==============================================')

    # 预制处理
    if len(args) > 0 and 'author' == args[0]:
        print_success('%s <%s>' % (AUTHOR_NAME, AUTHOR_EMAIL))
        exit()
    if len(args) > 0 and 'dir' == args[0]:
        print_success('%s' % SCRIPT_DIR)
        exit()
    if len(args) > 0 and 'open' == args[0]:
        os.system('open %s' % SCRIPT_DIR)
        exit()
    if len(args) > 0 and 'path' == args[0]:
        print_success('%s/%s' % (SCRIPT_DIR, SCRIPT_BASENAME))
        exit()
    if len(args) > 0 and 'summary' == args[0]:
        print_success('%s' % SCRIPT_DESC)
        exit()
    if len(args) > 0 and 'version' == args[0] or 'version' in opts:
        print_success('%s' % SCRIPT_VERSION)
        exit()
    if len(args) > 0 and 'wiki' == args[0]:
        print_success('说明文档: %s' % README_URL)
        exit()
    if 'h' in opts or 'help' or len(args) <= 0 in opts:
        help()
        exit()
